...
SELECT NVL(MAX(QSTEP),0) + 1 FROM QNA 
    WHERE QGROUP = 4;
...
SELECT A.*, (SELECT ANAME FROM ADMIN WHERE AID=A.AID) ANAME ,(SELECT MNAME FROM MEMBER WHERE MID=A.MID) MNAME 
    FROM (SELECT ROWNUM RN, L.* FROM 
    (SELECT * FROM QNA  WHERE MID= 'admin1' ORDER BY QNO DESC) L) A
    WHERE RN BETWEEN 1 AND 10;
--2. 글 갯수
...
INSERT INTO QNA (QNO ,QTITLE ,QCONTENT,QGROUP ,QFILE ,QIP ,AID, MID)
    VALUES(QNA_SEQ.NEXTVAL, 'TEST', 'TEST', QNA_SEQ.CURRVAL, NULL, '0.0.0.0',NULL,'ccc1');
INSERT INTO QNA (QNO ,QTITLE ,QCONTENT,QGROUP ,QFILE ,QIP ,AID, MID)
...
DELETE FROM QNA WHERE AID='admin1';
